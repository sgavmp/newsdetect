...
public interface StatisticsRepository extends CrudRepository<Statistics, Date> {
	...
	@Query(value="SELECT alert_detect_id,title,fecha,sum(num) num, avg(score_avg) score_avg  FROM alert_score where DATE_FORMAT(fecha, '%Y-%m-%d' ) <= DATE_FORMAT(:from, '%Y-%m-%d' ) and DATE_FORMAT(fecha, '%Y-%m-%d' ) > DATE_FORMAT(:to, '%Y-%m-%d' ) group by title order by score_avg desc limit :limit",nativeQuery = true)
	...
	@Query(value="SELECT alert_detect_id,title,fecha,sum(num) num, avg(score_avg) score_avg  FROM risk_score where DATE_FORMAT(fecha, '%Y-%m-%d' ) <= DATE_FORMAT(:from, '%Y-%m-%d' ) and DATE_FORMAT(fecha, '%Y-%m-%d' ) >  DATE_FORMAT(:to, '%Y-%m-%d' ) group by title order by score_avg desc limit :limit",nativeQuery = true)
	...
	List<Statistics> readAllByfecha(Date date);
